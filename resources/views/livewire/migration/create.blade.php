<form class="flex flex-col sm:flex-row" wire:submit.prevent="create">
    ...
    <div class="sm:w-1/4 w-full flex flex-col items-center mt-3 sm:mt-0 ">
        <input type="text" class="focus:outline-none focus:border-gray-400 border border-gray-200 rounded py-2 px-4 w-11/12 @error('connection') border-red-300 @enderror" wire:model.lazy="connection" placeholder="Connection Name">
        @error('connection')
            <span class="px-2 text-sm text-red-600">{{ $message }}</span>
        @enderror
    </div>
